...
def rename_files_in_photo(photo_dir):
    ...
    for root, _, files in os.walk(photo_dir):
        ...
        print(path_parts)
        if len(path_parts) < 5:  # 确保目录至少包含 "Photo\A\22.23\子分类"
            continue

        if len(path_parts) == 5:
            parent_category = path_parts[-2]  # A, B, C, D
            category_number = path_parts[-1]
        ...
        for file in files:
            ...
            if len(path_parts) == 7:  # 存在子子分类
                ...
                new_name = f"{category_number}-{mod_time_str}-{parent_category}{file_ext}"
...
# 指定 Photo 目录路径
photo_dir = r"D:\\Photo"  # 你的 Photo 目录路径
